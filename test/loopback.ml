...
let recv flow buf =
  ...
  then ( flow.c <- true ; Lwt.return_ok `End_of_flow )
  else
...
let send flow str =
  if flow.c
  then ( Fmt.epr "WANT TO SEND %S.\n%!" (Cstruct.to_string str) ; Lwt.return_error `Closed )
  else
    ( flow.o <- Cstruct.append flow.o str
    ; Lwt.return_ok (Cstruct.len str) )

let close flow = flow.c <- true ; Lwt.return_ok ()
