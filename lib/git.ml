...
= struct
  ...
  let fs =
    ...
    { Thin.create= (fun path -> Append.create path >|= R.reword_error (R.msgf "%a" Append.pp_error))
    ; Thin.append= Append.append
    ; Thin.map= Append.map
    ; Thin.close= (fun fd -> Append.close fd >|= R.reword_error (R.msgf "%a" Append.pp_error)) }

  (* XXX(dinosaure): abstract it? *)
  let digest
    : kind:[ `A | `B | `C | `D ] -> ?off:int -> ?len:int -> Bigstringaf.t -> Uid.t
  ...
  let run_idx ~dst ~pack entries =
    ...
    let rec go = function
      ...
      | `Ok -> Lwt.return_ok () in
    go (Enc.encode encoder `Await) >>? fun () ->
    Append.close fd
  ...
  let run ~light_load ~heavy_load stream ~src ~dst ~idx =
    ...
    run_idx ~dst:idx ~pack entries >|= R.reword_error (R.msgf "%a" Append.pp_error) >>? fun () ->
    Lwt.return_ok pack
  ...
  let fetch ~resolvers (access, light_load, heavy_load) store uri ?(version= `V1) ?(capabilities = []) want ~src ~dst ~idx =
    let open Rresult in
    let open Lwt.Infix in
    ...
    | `V1, Some "git", Some domain_name, path ->
      ...
      let run () =
        ...
        >>= fun (refs, idx) -> match refs, idx with
  ...
  let push ~resolvers (access, light_load, heavy_load) store uri ?(version= `V1) ?(capabilities= []) cmds =
    let open Rresult in
    match (version, Uri.scheme uri, Uri.host uri, Uri.path uri) with
    | `V1, Some "git", Some domain_name, path ->
      ...
      let domain_name = Domain_name.(host_exn <.> of_string_exn) domain_name in
      let run () =
        push ~resolvers ~capabilities path cmds domain_name store access push_cfg
